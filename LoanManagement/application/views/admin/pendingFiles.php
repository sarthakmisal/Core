	...
	<div class="row">
		<div class="col-md-12">
			<table class="table w-100 table-hovered table-stripped table-bordered">
				...
				<tbody>
					<?php foreach($pendingFiles as $key=>$row): ?>
						<tr>
							<td><?= $key+1?></td>
							<td><?=$row['c_name']?></td>
							<td><?=$row['c_phone']?></td>
							...
							<td style="white-space:nowrap;">
								...
								<button class="btn"><i class="fa fa-pen text-primary"></i></button>
								<a onclick="return confirm('Are you sure?')" href="<?=base_url('admin/deleteFile/').$row['f_id']?>">
