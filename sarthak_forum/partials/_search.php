...
if(isset($_GET['search'])){
    ...
    if(mysqli_fetch_assoc($datac)==null && mysqli_fetch_assoc($data)==null){
        ...
        ?>
        <h1 class="font-weight-bolder p-3">Search Results for "<em><?=$_GET['search']?></em>"</h1>
        <?php
...
while($row2=mysqli_fetch_assoc($datac)){
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 ">
                <a href="/partials/_thread.php?id=<?=$row2['cm_t_id']?>"><h4 class="text-primary mt-4 mb-0"><?= $row2['cm_content'] ?></h4></a>
            </div>
        </div>
    </div>
    <?php
