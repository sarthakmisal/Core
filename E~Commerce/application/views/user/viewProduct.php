    ...
    <div class="row">
            ...
            <div class="row m-2 my-5">
                <div class="col-md-3 overflow-hidden ">
                    ...
                    foreach ($data as $key => $row) {
                        ?>
                        <img onclick="changeImg('<?= base_url('uploads/') . $row ?>')" id="kit" class="my-2"
                            src="<?= base_url('uploads/') . $row ?>" width="120vw" alt="">
                        <?php
                    }
                    ?>
                </div>
                <div class="col-md-9">
                    <img id="iki" src="<?= base_url('uploads/') . $data[0] ?>" width="100%" alt="">
                </div>
            </div>
        </div>
        <script>
            data = '<?= $product[0]['p_images'] ?>'.split("&&")
            i = 0
            interval = setInterval(() => {
                i %= data.length
                iki.src = '<?= base_url('uploads/') ?>' + data[i]
                i++
            }, 2000)
        ...
        <div class="col-md-5">
            <div class="row my-5">
                <div class="col-md-12">
                    <div
                        style="font-weight:bold;color:white;padding:5px 38px;background-image:linear-gradient(45deg,white 6%, black 6%,black 22%,white 17%)">
                        <?= $product[0]['p_label'] ?>
                    </div>
                </div>
                <div class="col-md-12">
                    <h1 class="font-weight-bold"><?= $product[0]['p_name'] ?></h1>
                </div>
                <div class="col-md-12">
                    <h6 style="letter-spacing:5px" class="h2 font-weight-light">
                        &#8377;&nbsp;<?= number_format($product[0]['p_price']) ?>&nbsp;/-</h6>
                </div>
                <div class="col-md-12 my-2">
                    ...
                    if (!isset($cart[0])) {
                        ...
                        <a href="<?= base_url('user/addtoCart/') . $product[0]['p_id'] ?>">
                            <button class="btn btn-dark btn-lg my-3" style="border-radius:0px"><svg version="1.0" style="position:relative; top:-4px;right:2px;
                            xmlns=" http://www.w3.org/2000/svg" width="25" height="25"
                                    viewBox="0 0 1280.000000 1211.000000" preserveAspectRatio="xMidYMid meet">
                                    <metadata>
                                        Created by potrace 1.15, written by Peter Selinger 2001-2017
                                    </metadata>
                                    <g transform="translate(0.000000,1211.000000) scale(0.100000,-0.100000)" fill="white"
                                        stroke="none">
...
-78 339 29 339 199 0 54 -28 125 -63 163 -57 62 -176 87 -253 55z" />
                        ...
                        ?>
                        <button onclick="decrease(quantity.value)" style="margin-top:-5px;font-size:1.5rem" class="btn btn-sm btn-dark rounded-0">-</button>&nbsp;
                        <input type="button" id="quantity" style="background:transparent;font-weight:bold;border:none" value="<?=$cart[0]['quantity']?>">&nbsp;
                        <button onclick="increase(quantity.value)" style="margin-top:-5px;font-size:1.5rem" class="btn btn-sm btn-dark rounded-0">+</button>
                        <?php
                    }
                    ?>
                </div>
                <div class="col-md-12">
                    <p><?= nl2br($product[0]['p_desc']) ?></p>
                </div>
            </div>
        </div>
...
<script>
    function increase(first){
        if(first<6) fetch('<?=base_url("user/increaseQuantity/").$cart[0]["c_id"]?>').then(res=>res.json()).then(data=>quantity.value=data)
    }
    function decrease(first){
        if(first>1) fetch('<?=base_url("user/decreaseQuantity/").$cart[0]["c_id"]?>').then(res=>res.json()).then(data=>quantity.value=data)
    }
</script>